...
static const int SELECT_METHOD[] = {
    0, 0, 1, 12, 12, 12, 12, 12, 12, 12, 12, 15, 15, 15, 8,  0, 1, 1, 2, 2, 4, 4,  13, 13, 14, 14, 15, 15, 15, 8,
    1, 1, 2, 2,  2,  4,  4,  14, 14, 14, 14, 15, 15, 15, 9,  1, 1, 2, 4, 4, 4, 4,  6,  6,  15, 15, 15, 15, 15, 9,
    1, 2, 2, 4,  4,  5,  5,  7,  7,  16, 16, 16, 11, 11, 10, 1, 2, 4, 4, 4, 5, 5,  7,  7,  16, 16, 16, 11, 11, 11,
    1, 2, 3, 3,  5,  5,  7,  7,  16, 16, 16, 16, 16, 11, 11, 1, 2, 3, 3, 5, 5, 17, 17, 17, 17, 16, 16, 16, 11, 11};

static const double HRANGE[] = {0.02, 0.06, 0.09, 0.125, 0.26, 0.4, 0.6, 1.6, 1.7, 2.33, 2.4, 3.36, 3.4, 4.8};

static const double ARANGE[] = {0.025, 0.09, 0.15, 0.36, 0.5, 0.9, 0.99999};

static const double ORD[] = {2, 3, 4, 5, 7, 10, 12, 18, 10, 20, 30, 0, 4, 7, 8, 20, 0, 0};

static const int METHODS[] = {1, 1, 1, 1, 1, 1, 1, 1, 2, 2, 2, 3, 4, 4, 4, 4, 5, 6};

static const double C[] = {0.99999999999999999999999729978162447266851932041876728736094298092917625009873,
...
static const double PTS[] = {0.35082039676451715489E-02, 0.31279042338030753740E-01, 0.85266826283219451090E-01,
...
static const double WTS[] = {0.18831438115323502887E-01, 0.18567086243977649478E-01, 0.18042093461223385584E-01,
...
static double owens_t_norm1(double x) { return erf(x / sqrt(2)) / 2; }

static double owens_t_norm2(double x) { return erfc(x / sqrt(2)) / 2; }
...
double owens_t(double h, double a) {
    ...
    if (fabs_a == INFINITY) {
        ...
        if (fabs_ah <= 0.67) {
            ...
            result = 0.25 - normh * normah - owens_t_dispatch(fabs_ah, (1 / fabs_a), h);
        } else {
            normh = owens_t_norm2(h);
            normah = owens_t_norm2(fabs_ah);
            result = (normh + normah) / 2 - normh * normah - owens_t_dispatch(fabs_ah, (1 / fabs_a), h);
        }
    }
