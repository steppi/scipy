...
static double pow2(double a, double b, int m) { return dd_to_double(pow_D(add_dd(a, b), m)); }
...
static ThreeProbs _smirnov(int n, double x) {
    ...
    }
    {
        ...
        bUseUpperSum =
            (bUseUpperSum || ((n >= SM_UPPERSUM_MIN_N) && (nUpperTerms <= SM_UPPER_MAX_TERMS) && (x <= 0.5 / sqrt(n))));
    }
